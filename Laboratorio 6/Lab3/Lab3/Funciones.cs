...
namespace Funciones
{
    public class Class1
    {
        ...
        {
            string sql = $"select * from dbo.EstudianteTarea where Email = '{email}'";
            SqlDataAdapter adapter = new SqlDataAdapter(sql, conexion);
            DataSet dataset = new DataSet();
        ...
        public static DataTable getTareaGenerica(String tarea)
        {
            string sql = $"select * from dbo.TareaGenerica where codAsig='{tarea}'";
            SqlDataAdapter adapter = new SqlDataAdapter(sql, conexion);
            DataSet dataset = new DataSet();
            adapter.Fill(dataset, "TareaGenerica");
            DataTable data = dataset.Tables["TareaGenerica"];
        ...
        {
            string sql = $"select codigoAsig from dbo.GrupoClase, dbo.EstudianteGrupo where dbo.GrupoClase.codigo=dbo.EstudianteGrupo.Grupo and dbo.EstudianteGrupo.email='{email}'";
            SqlDataAdapter adapter = new SqlDataAdapter(sql, conexion);
            DataSet dataset = new DataSet();
            adapter.Fill(dataset, "EstudianteTarea");
        ...
        public static bool InstanciarTarea(string email, string codigo, int horasEstimadas, int horasReales, DataSet dataset)
        {
            try
            {
                string sql = $"insert into  dbo.EstudianteTarea values('{email}','{codigo}',{horasEstimadas}, {horasReales})";
                SqlDataAdapter adapter = new SqlDataAdapter();
                adapter.InsertCommand = new SqlCommand(sql, conexion);
        ...
        public static DataTable getAsignaturasProfesor(String email)
        {
            string sql = $"SELECT GrupoClase.codigoAsig FROM GrupoClase INNER JOIN ProfesorGrupo ON GrupoClase.codigo = ProfesorGrupo.codigoGrupo WHERE(ProfesorGrupo.email = '{email}')";
            SqlDataAdapter adapter = new SqlDataAdapter(sql, conexion);
            DataSet dataset = new DataSet();
            adapter.Fill(dataset, "GrupoClase");
            DataTable data = dataset.Tables["GrupoClase"];
            return data;
        }
