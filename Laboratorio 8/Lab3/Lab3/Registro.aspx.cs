...
namespace Registro_de_Usuario
{
    public partial class Registro : System.Web.UI.Page
    {
        ...
        {
            if (Label8.Text == "Correo no matriculado")
            {
                Label7.Text = "Debe de introducir un correo matriculado para poder registrarse";
            }
            else
            {
                ...
                if (esEmailUsado)
                {
                    ...
                    {
                        Label7.Text = "Se ha registrado, pero ha ocurrido un error en el envio del email de confirmacion. Hable con un administrador";
                    }
                }
                else
                {
        ...
        {
            Lab3.matriculas.Matriculas service = new Lab3.matriculas.Matriculas();
            if (service.comprobar(TextBox1.Text) == "NO")
            {
                Label8.Text = "Correo no matriculado";
            }
            else
            {
                Label8.Text = "Correo matriculado";
            }
        }
    }
}